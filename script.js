...
function initCamera(scene, { width, height, renderer }) {
    if (!renderer)
        throw new Error('???');
    ...
    for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j <= 26; j++) {
            ...
            const neighbours = Array.from(filled).filter((filledPoint) => {
                const [filledPointX, filledPointY, filledPointZ] = getCoords(filledPoint);
                return ((filledPointX === x &&
                    filledPointY === y &&
...
function initShapes(scene, { config }) {
    if (!config)
        throw new Error('???');
    ...
    const frame = (props) => {
        ...
        if (props.hasChanged) {
            ...
            shapeMaterial.uniforms.uActiveColor.value = new THREE.Color(activeColor.r, activeColor.g, activeColor.b);
            const inactiveColor = props.config.shapeInactiveColor;
            shapeMaterial.uniforms.uInactiveColor.value = new THREE.Color(inactiveColor.r, inactiveColor.g, inactiveColor.b);
        }
        const highlighter = scene.getObjectByName('highlighter');
        if (!(highlighter instanceof THREE.Mesh))
            throw new Error('???');
...
function initHighlighter(scene, _props) {
